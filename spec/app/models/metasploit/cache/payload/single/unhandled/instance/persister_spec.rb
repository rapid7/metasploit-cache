RSpec.describe Metasploit::Cache::Payload::Single::Unhandled::Instance::Persister, type: :model do
  ...
  context 'resurrecting attributes' do
    context '#persistent' do
      ...
      subject(:persistent) {
        payload_single_unhandled_instance_persister.persistent
      }
      ...
      let(:payload_single_unhandled_instance_persister) {
        described_class.new(
  ...
  context '#persist' do
    ...
    subject(:persist) {
      payload_single_unhandled_instance_persister.persist(*args)
    }
    ...
    context 'with :to' do
      ...
      it 'does not access default #persistent' do
        expect(payload_single_unhandled_instance_persister).not_to receive(:persistent)
    ...
    context 'without :to' do
      ...
      it 'defaults to #persistent' do
        expect(payload_single_unhandled_instance_persister).to receive(:persistent).and_call_original

        persist
      end

      it 'uses #batched_save' do
        expect(payload_single_unhandled_instance_persister.persistent).to receive(:batched_save).and_call_original
