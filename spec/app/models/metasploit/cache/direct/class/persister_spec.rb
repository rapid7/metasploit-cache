RSpec.describe Metasploit::Cache::Direct::Class::Persister, type: :model do
  ...
  context 'resurrecting attributes' do
    context '#persistent' do
      subject(:persistent) {
        direct_class_persister.persistent
      }
  ...
  context '#persist' do
    subject(:persist) do
      direct_class_persister.persist(*args)
    end
    ...
    context 'with :to' do
      ...
      it 'does not access default #persistent' do
        expect(direct_class_persister).not_to receive(:persistent)
      ...
      context 'without #rank' do
        it 'does attempt to save' do
          expect(Metasploit::Cache::Module::Class::Persister::Rank).to receive(:synchronize).and_return(expected_direct_class)
    ...
    context 'without :to' do
      ...
      it 'defaults to #persistent' do
        expect(direct_class_persister).to receive(:persistent).and_call_original
      ...
      context 'with #rank' do
        it 'uses #batched_save' do
          expect(direct_class_persister.persistent).to receive(:batched_save).and_call_original
      ...
      context 'without #rank' do
        it 'does attempt to save' do
          expect(Metasploit::Cache::Module::Class::Persister::Rank).to receive(:synchronize).and_return(direct_class_persister.persistent)
          expect(direct_class_persister.persistent).to receive(:batched_save)
