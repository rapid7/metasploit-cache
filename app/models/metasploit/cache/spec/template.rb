# Processes {EXTENSION '.rb.erb'} templates to create {Metasploit::Cache::Module::Ancestor#contents} that contain the
# same metadata as the {Metasploit::Cache::Module::Ancestor},
# {Metasploit::Cache::Module::Class}, {Metasploit::Cache::Module::Instance} and associations for those contents.  This
# ensures that when the {Metasploit::Cache::Module::Ancestor#contents} are loaded in metasploit-framework, the same
# metadata instances are derived from the contents ensuring idempotency of the contents and metadata parsing loop.
class Metasploit::Cache::Spec::Template < Metasploit::Model::Base
  ...
  class << self
    ...
    #
    # @return [Pathname] Defaults to 'spec/support/templates/metasploit/cache' under Metasploit::Cache::Engine.root.
    def root
      @@root ||= Metasploit::Cache::Engine.root.join('spec', 'support', 'templates', 'metasploit', 'cache')
    end
