...
RSpec.describe Metasploit::Model::Search::Operator::Deprecated::Authority do
  subject(:operator) do
    described_class.new(
  ...
  context '#name' do
    ...
    it 'should be #abbreviation' do
      expect(name).to eq(abbreviation)
    end
  end
  ...
  context '#operate_on' do
    ...
    before(:each) do
      allow(operator).to receive(:operator)
                             .with('authorities.abbreviation')
                             .and_return(authorities_abbreviation_operator)
