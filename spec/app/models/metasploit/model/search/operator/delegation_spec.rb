...
RSpec.describe Metasploit::Model::Search::Operator::Delegation do
  subject(:operator) do
    described_class.new(
        :klass => klass
    )
  ...
  context '#operator' do
    ...
    before(:each) do
      search_operator_by_name = self.search_operator_by_name

      klass.define_singleton_method(:search_operator_by_name) do
