...
RSpec.describe Metasploit::Cache::Search::Operator::Deprecated::Platform do
  subject(:operator) do
    described_class.new(
        :klass => klass,
        :name => name
  ...
  context '#children' do
    ...
    before(:each) do
      allow(operator).to receive(:operator).with(
          'platforms.fully_qualified_name'
      ).and_return(
          platforms_fully_qualified_name_operator
      )

      allow(operator).to receive(:operator).with('targets.name').and_return(targets_name_operator)
    end
