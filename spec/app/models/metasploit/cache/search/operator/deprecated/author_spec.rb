RSpec.describe Metasploit::Cache::Search::Operator::Deprecated::Author do
  subject(:operator) do
    described_class.new(
        :klass => klass
    )
  ...
  context '#children' do
    ...
    before(:each) do
      allow(operator).to receive(:operator).with('authors.name').and_return(authors_name_operator)
      allow(operator).to receive(:operator).with('email_addresses.domain').and_return(email_addresses_domain_operator)
      allow(operator).to receive(:operator).with('email_addresses.local').and_return(email_addresses_local_operator)
    end
