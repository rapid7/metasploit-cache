RSpec.describe Metasploit::Cache::Encoder::Instance::Persister, type: :model do
  it_should_behave_like 'Metasploit::Cache::Module::Persister'

  context 'resurrecting attributes' do
    context '#persistent' do
      subject(:persistent) {
        encoder_instance_persister.persistent
      }
      ...
      let(:encoder_instance_persister) {
        described_class.new(
      ...
      it 'has #encoder_class matching pre-existing Metasploit::Cache::Encoder::Class' do
        expect(persistent.encoder_class).to eq(existing_encoder_instance.encoder_class)
      end
    end
  end
  ...
  context '#persist' do
    ...
    subject(:persist) {
      encoder_instance_persister.persist(*args)
    }
    ...
    context 'with :to' do
      ...
      it 'does not access default #persistent' do
        expect(encoder_instance_persister).not_to receive(:persistent)
      ...
      context 'batched save' do
        context 'failure' do
          ...
          it 'tags log with Metasploit::Cache::Module::Ancestor#real_path' do
            ...
            expect(logger_string_io.string).to include("[#{encoder_instance.encoder_class.ancestor.real_pathname.to_s}]")
          end
          ...
          it 'logs validation errors' do
            ...
            expect(logger_string_io.string).to include("Could not be persisted to #{encoder_instance.class}: #{full_error_messages}")
          end
        end
    ...
    context 'without :to' do
      ...
      it 'defaults to #persistent' do
        expect(encoder_instance_persister).to receive(:persistent).and_call_original

        persist
      end

      it 'uses #batched_save' do
        expect(encoder_instance_persister.persistent).to receive(:batched_save).and_call_original
